...
*   **Ulkoiset TSPLIB-tiedostot:** Useita testejä käyttää parametrisoidusti `rand4.tsp` ... `rand12.tsp` -tiedostoja `problems/random`-kansiosta ja vastaavia `.opt.tour`-tiedostoja `solutions/exact`-kansiosta sekä muita TSPLIB-instansseja `problems/tsplib95`-kansiosta.
